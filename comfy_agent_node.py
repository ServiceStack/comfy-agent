...
def register_agent():
    ...
    if isinstance(response.require_pip, list):
        if len(g_installed_pip_packages) == 0:
            print(f"Installing required pip packages: {len(response.require_pip)}")
    ...
    if isinstance(response.require_nodes, list):
        if len(g_installed_custom_nodes) == 0:
            print(f"Installing required custom nodes: {len(response.require_nodes)}")
    ...
    if isinstance(response.require_models, list):
        if len(g_installed_models) == 0:
            print(f"Downloading required models: {len(response.require_models)}")
...
def send_update_status(wait=1):
    ...
    if g_status is not None:
        ...
        try:
            ...
            print(f"Error sending update status: {e}")

def save_installed_items(file:str, items:list):
    with open(os.path.join(get_user_directory(), file), "w") as f:
        f.write("\n".join(items))

def load_installed_items(file:str):
    install_path = os.path.join(get_user_directory(), file)
    if not os.path.exists(install_path):
        return []
    try:
        with open(install_path, "r") as f:
            lines = f.read().splitlines()
            print(f"Loaded {len(lines)} installed items from {file}")
            printdump(lines)
            return lines
    except Exception as e:
        print(f"Error loading installed items from {file}: {e}")
        return []
...
def install_custom_node(repo_url):
    try:
        ...
        if os.path.exists(custom_node_path):
            print(f"{custom_node_path} already exists")
            append_installed_item("require-nodes.txt", g_installed_custom_nodes, repo_url)
            return

        print("Installing custom node: " + repo_name + " in " + custom_nodes_dir)
...
def download_model(save_to, url):
    try:
        ...
        if os.path.exists(save_to_path):
            print(f"{save_to_path} already exists")
...
def reboot():
    reboot_url = f"{g_server_url}/api/manager/reboot"
    try:
        global g_status
